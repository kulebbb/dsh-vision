...
/** Vision errors that must fail the request rather than degrade to a placeholder. */
const HARD_VISION_CODES = new Set([
  "VISION_UNAVAILABLE",
  "INVALID_CREDENTIAL",
  "MISSING_CREDENTIAL",
  "VISION_TIMEOUT",
...
export class VisionDeepSeekAdapter extends DeepSeekAdapter {
  ...
  async describeRef(ref, signal) {
    try {
      ...
      if (error?.code && HARD_VISION_CODES.has(error.code)) {
        throw error;
      }
