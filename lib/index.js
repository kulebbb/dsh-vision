...
export function apply(ctx, config = {}) {
  ...
  const connection = () => {
    ...
    try {
      // The settings schema materializes an absent `deepseek` object with
      // `models: []`; strip that so resolveAdapterOptions falls back to its
      // own default model catalog instead of serving an empty one.
      const ds = { ...(raw.deepseek ?? {}) };
      if (Array.isArray(ds.models) && ds.models.length === 0) delete ds.models;
      const next = resolveAdapterOptions(ds, launchEnvironmentOf(ctx));
      lastRaw = raw;
      lastGood = next;
      return next;
    } catch (error) {
